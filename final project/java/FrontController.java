...
public class FrontController {
	...
	public String joinMadi(MemberVO memberVO, HttpServletResponse response, HttpSession session) {
	...
	public void updateNotification(String notice_id, String followAdmit) {
		...
		if (notificationVO.getNotice_type().equals("팔로우 신청")) {
			if (followAdmit.equals("수락")) {
				...
				MemberVO fromMember = memberDAOService.getUserInfoById(notificationVO.getNotice_to());
				acceptNotice.setContent(fromMember.getUser_name() + "님께서 회원님의 팔로우 신청을 수락하셨습니다");
				...
				MemberVO fromMember = memberDAOService.getUserInfoById(notificationVO.getNotice_to());
				rejectNotice.setContent(fromMember.getUser_name() + "님께서 회원님의 팔로우 신청을 거부하셨습니다");
	...
	public String getNotification(Model model, String user_id) {
		ArrayList<NotificationVO> notificationList = notificationDAOService.getMyNoticeById(user_id.toString());
		model.addAttribute("notificationList", notificationList);
		return "notification";
	...
	public String likeBoard(UserLikeBoVO userLikeBoVO, String writer) {
		...
		if (vo == null) {
			...
			MemberVO fromMember = memberDAOService.getUserInfoById(userLikeBoVO.getUser_id());
			// 좋아요 알림 메시지 전송
			NotificationVO notificationVO = new NotificationVO();
			notificationVO.setNotice_to(writer);
			notificationVO.setNotice_from(userLikeBoVO.getUser_id());
			String content = fromMember.getUser_name() + "님이 회원님의 게시물을 좋아합니다.";
	...
	public String followRequest(MemberFollowVO memberFollowVO) {
		...
		MemberVO fromMember = memberDAOService.getUserInfoById(memberFollowVO.getUser_id());
		followRequest.setContent(fromMember.getUser_name() + "님께서 팔로우를 신청하셨습니다.");
