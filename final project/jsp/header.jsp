 ...
 <nav class="navbar navbar-default head header" data-spy="affix" data-offset-top="197">
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="myNavbar" align="center">
            <ul class="nav navbar-nav navbar" style="text-align: center; width:auto;">
            ...
            <ul class="nav navbar-nav navbar-right" style="vertical-align: middle;">
                ...
                <li>
                    <div class="dropdown" style="padding-top: 9px; padding-left: 2px;">
                        ...
                        <ul class="dropdown-menu" style="text-align:center; background-color:#F6F6F6;">
                            ...
                            <li>
                                <h4>
                                    <a href="./mypage.do"><p class="text-primary"><%=memberVO.getUser_name()%></p></a>
                                </h4>
                            </li>
                            <li>
                ...
                <li onclick="postListDo();">
                    <button type="button" class="btn form header" style="padding-top: 10px; margin-top:10px; margin-left:1px;" >
                        <span class="glyphicon glyphicon-picture color"></span>
                    </button>
                </li>
          	</ul>
       	</div>
...
<div class="modal fade" id="fridge" tableindex="-1" role="dialog"
	aria-labelledy="modallabel" style="height:auto;">
				...
				<div class="tab-content">
					<div id="listOfIrdnt" class="tab-pane fade in active">
						<div class="row">
							...
							<!-- 왼쪽 카테고리 -->
							<!--  가운데 리스트 나열 -->
							<div class="col-sm-6" style="background-color: #FFEAEA; text-align: center; padding-right:0;">
								<form id="IrdntList">
									<div class="tab-content" style="overflow-y:scroll; height:272px;">
										<!-- 메뉴1 빵류 -->
										<div id="menu1" class="tab-pane fade in active">
											<ul class="ul hori">
												<li class="li hori" onclick="irdntClickListener(this);">
								...
								<!-- 가운데 리스트 -->
								<!-- 오른쪽 내 재료들 저장칸 -->
								<div class="col-sm-4" style="overflow-y:scroll; height:272px;">
									<div class="row">
										<div class="col-sm-12 text-center text-muted" style="background-color: #FFEAEA; font-color:#FFFFFF;">
											<b>추가할 재료 목록</b>
										</div>
										<form id="myIrdntListForm">
											<ul class="ul hori" id="myIrdntList">
												<!-- 클릭한 재료가 넘어오는 곳 -->
											</ul>
										</form>
									</div>
								...
								<div class="modal-footer">
									<button type="button" class="btn btn-default" onclick="addIrdnt();">
										<p class="bg-danger" style="font-color:#FFFFFF; width=200px; margin-bottom: 0px;">
											<strong>저장</strong>
										</p>
									</button>
					...
					<div id="listOfMyFridge" class="tab-pane fade">
						<div class="row">
							...
							<div class="col-sm-8" style="background-color: #FFEAEA; text-align: center; padding-right:0;">
								<ul class="ul hori" id="myFridge">
									<!-- DB에 현재 저장되어 있는 내 냉장고 재료들 -->
<%
									for(int i = 0; i < myIrdntList.size(); i++) {
										MemberBoxVO memberBoxVO = myIrdntList.get(i);
%>
										<script>
											var my_irdnt = "<%=memberBoxVO.getMy_irdnt()%>";
											var src = $("#IrdntList").find("li").find("input[value='"+my_irdnt+"']").siblings("img").attr("src");
											var li = "<li class='li hori text-muted' style='cursor:pointer;' onclick='myFridgeClickListener(this);'>"
													 + "<img src='"+src+"' style='width: 35px; height: 35px;'><br>"
													 + "<input type='hidden' name='my_irdnt' value='"+my_irdnt+"'>"
													 + "<strong>"+my_irdnt+"</strong>"
													 + "</li>";
											$("#myFridge").append(li);
										</script>										
<%
...
<div class="modal modal-center fade" id="myModal" tableindex="-1" style="margin-top:100px;border-radius: 8px;" 
	...
	<div class="modal-dialog modal-size modal-center" role="document">
		<div class="modal-content modal-size" style="width: 350px; border-radius: 8px;">
			<!-- 알림, 메시지 탭 -->
...
/* 
	...
	function updateNotification(notice_id, followAdmit) {
		var id = "n"+notice_id;
	...
	// 가운데 리스트 클릭 시 우측으로 이동
	function irdntClickListener(e) {
		var irdnt = $(e).find("input").val();
		var src = $(e).find("img").attr("src");
		var li = "<li class='li hori text-muted w3-animate-opacity' style='cursor:pointer;' onclick='myFridgeClickListener(this);'>"
			 + "<img src='"+src+"' style='width: 35px; height: 35px;'><br>"
			 + "<input type='hidden' name='my_irdnt' value='"+irdnt+"'>"
			 + "<strong>"+irdnt+"</strong>"
			 + "</li>";
		if(findIrdntInFridge(irdnt)) {
			alert("내 냉장고에 추가되어 있는 재료입니다.");
		} else {
			if(findIrdntInList(irdnt)) {
				alert("추가 목록에 추가되어 있는 재료입니다.");	
			} else {
				document.getElementById("myIrdntList").innerHTML += li;
			}	
		}	 
	}
	function findIrdntInList(irdnt) {
		var isExist = $("#myIrdntList").find("li").find("input[value='"+irdnt+"']").val();
	...
	function findIrdntInFridge(irdnt) {
		var isExist = $("#myFridge").find("li").find("input[value='"+irdnt+"']").val();
		if(isExist===undefined) {
			return false;
		} else {
			return true;
	...
	function addIrdnt() {
		var param = $("#myIrdntListForm").serialize();
		$.ajax({
			url: "./insertMemberBox.do",
			type: "POST",
			data : param,
			success: function(status) {
				var addedIrdnt = $("#myIrdntListForm").find("li");
				$("#myFridge").append(addedIrdnt);
				$("#myIrdntList").empty();
			}

		});
	}
	function myFridgeClickListener(e) {
			var irdnt = $(e).find("input").val();
			$.ajax({
				url: "./deleteMemberBox.do",
				type: "POST",
				data: {
					my_irdnt: irdnt
				},
