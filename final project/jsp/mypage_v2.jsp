...
<%
	...
	request.setAttribute("genderchk", memberVO.getUser_gender());
%>
<html>
<head>
...
<script>
	...
	/* 팔로우 요청 */
	function followRequest(user_id, following_user_id, user_img, following_user_img, name) {
		// 내가 날 팔로우할 순 없음
		...
		$.ajax({
			...
			success : function(data) {
				if (data == 0) {
					...
					alert("현재 " + name + "님의 요청 수락을 기다리고 있습니다.");
				} else if (data == 2) {
					alert(name + "님께 팔로우를 신청하셨습니다.");
				}
			}
		});
	}
	...
	function blockMember(user_id, block_user_id) {		
		var r = confirm(block_user_id + "님을 정말로 차단하실 건가요?");
...
%>	      
				...
				<div id="updateMember" style="display:none;">
					<div class="w3-container w3-card-4 w3-white w3-round w3-margin w3-animate-opacity">
						<br>
						<form action="./updateInfo.do" method="post">
							<div class="form-group">		
								<hr/>
								<font class="madi_subtitle2">아이디</font><input type="text" class="form-control" id="join_user_id" placeholder="아이디" name="user_id" maxlength="20" value="<%=memberVO.getUser_id() %>" disabled="disabled" />
								<hr/>
					      		<font class="madi_subtitle2">이름</font><input type="text" class="form-control" id="join_user_name" placeholder="이름" name="user_name" value="<%=memberVO.getUser_name() %>" disabled="disabled"/>
								<hr/>
					      		<font class="madi_subtitle2">비밀번호</font><input type="password" class="form-control" id="join_user_pw" placeholder="비밀번호" name="user_pw" maxlength="20" value="<%=memberVO.getUser_pw()%>"/>
								<hr/>
					      		<font class="madi_subtitle2">이메일</font><input type="email" class="form-control" id="join_user_email" placeholder=" 이메일" name="user_email" value="<%=memberVO.getUser_email()%>"/>
								<hr/>
...
%>
					<div class="recfol w3-animate-opacity" style="display:block;">
	        			...
	        			<div class="w3-container w3-padding" style="position:relative;">
	          				...
	          				<p>
	          					<button type="button" class="btn btn-danger btn-sm"	onclick="followRequest('<%=memberVO.getUser_id() %>','<%=recfol.getUser_id()%>', '<%=memberVO.getUser_img()%>', '<%=recfol.getUser_email() %>', '<%=recfol.getUser_name() %>')" style="border-radius: 20px;">
										<strong>팔로우</strong>
								</button>
	          				</p>
	        			</div>
